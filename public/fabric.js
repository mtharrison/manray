...
(function(){
  ...
  function fire(eventName, options) {
    ...
    var skip = false; // Added by Bob D, 12-nov-13 to catch double-firings on mobile devices // See ticket: #533
    if(eventName == 'mousedown' && options && options.e)
    {
      //console.log(options.e);
      skip = !(options.e.type == ((fabric.isTouchSupported && options.e.touches && options.e.touches.length > 0) ? 'touchstart' : 'mousedown'));
    }
    if(!skip)
    {
    listenersForEvent[i](options || { });
    }
    //original code: listenersForEvent[i](options || { });
    }
  }
...
fabric.Collection = {
  ...
  iterativelyGetObjectsInsideRect: function(rectCoords, excludeObjs) {
    ...
    var intersectingObjects = [],
        allObjs = this._objects.filter(function(el){ return el.visible && excludeObjs.indexOf(el) === -1 }),
    ...
    while (foundStack.length > 0) {
        ...
        for (i = 0; i < allObjs.length; i++) {
            ...
            if ((fabric.util.doCoordinatesIntersect(foundObj.getCoords(), objToTest.getCoords()) && 
              ...
              // If the object is a text object, we should also
              // clear its bounding rectangle because text drawn
              // on top of text doesn't render correctly because of
              // anti-aliasing
              //
              // @todo this function is becoming unweildy and needs
              // refactoring attention
              if(objToTest.hasOwnProperty('text')){
                objToTest.submitBoundsAsDirtyRect();
              }
            } else {
    ...
    intersectingObjects.shift(); // This is our original dummyObject
...
(function() {
  ...
  function doCoordinatesIntersect(coords1, coords2) {
  ...
  fabric.util.doCoordinatesIntersect = doCoordinatesIntersect;
...
(function () {
  ...
  */
  imageSmoothingEnabled: true,
    ...
    lockRender: function() {
      this.renderLockStack.push(true);
      return this;
    },

    unlockRender: function() {
      this.renderLockStack.pop();
      return this;
    },

    isRenderLocked: function() {
      return this.renderLockStack.length > 0;
    },
  ...
  _setImageSmoothing: function(){
    ...
    setDimensions: function(dimensions) {
      ...
      var numKeys = Object.keys(dimensions).length;
      var thisKey = 0;

      for (var prop in dimensions) {
        thisKey++;
        this._setDimension(prop, dimensions[prop], thisKey === numKeys - 1);
      }
     ...
     * @chainable true
     */
    _setDimension: function (prop, value, skipRender) {
      // Modified for oversampling
      // 
      skipRender = skipRender || false;



      var scaledValue = value * (globalOversampleScale || 1);

	  // Don't update if the property hasn't changed, otherwise the DOM gets
	  // hammered which is very inefficient (and actually causes Chrome (36)
	  // on S3 to crash out completely). - Kev Porter, 20140804
      var changed = (this[prop] != scaledValue);

      // For some reason, on some games (GTP at least) on the first call to set a property
      // the width (or height) is already set, which messes up the check for changes.
      // This only happens on the 'new' dev environment which has the surrounding HTML/CSS
      // more similar to production.
      // I couldn't find the underlying cause of this (although possibly due to the
      // game container now being in a fixed width container), so a workaround is to keep
      // flags to say if this is the first call to set a property. If it is, perform
      // the actions regardless of whether the value appears to have changed.
      // - Kev Porter, 20140805

      if ( !this._setDimensionCalledBefore )
      {
        this._setDimensionCalledBefore = { width: false, height: false };
      }

      var firstCall = !this._setDimensionCalledBefore[prop];
      var doSetProperty = (changed || firstCall);

      if ( doSetProperty )
      {

         this.lowerCanvasEl[prop] = scaledValue;
         this.lowerCanvasEl.style[prop] = value + 'px';
		  

         if (this.upperCanvasEl) {
           this.upperCanvasEl[prop] = scaledValue;
           this.upperCanvasEl.style[prop] = value + 'px';
         }

         if (this.cacheCanvasEl) {
           this.cacheCanvasEl[prop] = scaledValue;
         }
         ...
         this[prop] = scaledValue;

         // Everything needs drawing again because we've globally modified rhe canvas
         this.setAllObjectsAsDirty();

      }

      // NB - this shouldn't be done on every call to this function, but retifying it breaks games
      // at the moment.
      // - Kev Porter, 20140805
      this.calcOffset();
      ...
      // Set 'called before' flag for this property.
      this._setDimensionCalledBefore[prop] = true;

      return this;
    },

    setAllObjectsAsDirty: function() {
    ...
    renderAll: function (allOnTop) {
      ...
      if(this.isRenderLocked()) return this;
      ...
      var totalRects = this.dirtyRects.length;
      var totalDirtyObjects = 0;
      var totalRendered = 0;
      ...
      if(this.performanceMode){
        ...
        for(var i = 0; i < this.dirtyRects.length; i++){
          ...
          additonalObjectsToRender = additonalObjectsToRender.concat(this.iterativelyGetObjectsInsideRect(
            rect, 
        ...
        totalDirtyObjects = additonalObjectsToRender.length
      ...
      for (var i = 0, length = this._objects.length; i < length; ++i) {
        if (!activeGroup ||
          ...
          if(this.performanceMode){
            if(this._objects[i] && this._objects[i].dirty || allOnTop){
      ...
      //console.log("Renderall was called and there was " + totalRects + " dirty rects, " + totalDirtyObjects + " object marked as dirty and " + totalRendered  + " objects rendered" );
      //console.trace();
...
(function() {
  ...
  var InteractiveMethods = /** @lends fabric.Canvas.prototype */ {
    ...
    _normalizePointer: function (object, pointer) {
      ...
      // Modified for oversampling
      return { x: x*(globalOversampleScale || 1), y: y*(globalOversampleScale || 1) };
    },
...
(function(){
  ...
  function mouseBound(evt, cvs) {
    var e = {
      ...
      type: evt.type,
      view: evt.view,
      webkitMovementX: evt.webkitMovementX,
      webkitMovementY: evt.webkitMovementY,
      which: evt.which,
      x: evt.x,
      y: evt.y// Duplicate e
    };
...
(function(global) {
  ...
  fabric.Object = fabric.util.createClass(/** @lends fabric.Object.prototype */ {
    ...
    set: function(key, value) {
      ...
      // If we're setting it to the same thing, it's not dirty
      if(this[key] !== value){
...
(function(global) {
  ...
  fabric.Image = fabric.util.createClass(fabric.Object, /** @lends fabric.Image.prototype */ {
    ...
    toObject: function(propertiesToInclude) {
      return extend(this.callSuper('toObject', propertiesToInclude), {
        src: this._element.src || this._originalImage._src,
...
fabric.Canvas.prototype._onMouseMove = fabric.Canvas.prototype.__onMouseMove = fabric.Canvas.prototype.__onMouseUp = function(e){
  return false;
};
...
// Modified set element so images don't resize on set element.
fabric.Image.prototype.setElement = function(element) {
